...
class PersonalCoursesController extends Controller{
    ...
    public function index($course){
        ...
        //Assignments Data
        list($activity,$submittedAssignments,$gradedAssignments,$asMarks) = $this->getAssignmentData($user_stmts);
        ...
        list($date_counts,$week_counts) = $this->getActivityOverTime($user_stmts,$today,$sDate,$duration);

        // Assignment Positioning on Progress bar
        list($subAssignments,$pendAssignments) = $this->getProgressbarPosition($course,$duration,$sDate,$submittedAssignments);

        //Previous Results Overview
        $prevResults = $this->getPrevResults($my_course[0]->cid);
